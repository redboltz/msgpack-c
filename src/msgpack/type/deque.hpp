...
inline std::deque<T>& operator>> (object<ForwardIterator> const& o, std::deque<T>& v)
{
...
template <typename T, typename ForwardIterator>
inline void operator<< (typename object<ForwardIterator>::with_zone& o, const std::deque<T>& v)
{
	o.type = type::ARRAY;
	if(v.empty()) {
