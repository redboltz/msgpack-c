...
inline std::list<T>& operator>> (object<ForwardIterator> const& o, std::list<T>& v)
{
...
template <typename T, typename ForwardIterator>
inline void operator<< (typename object<ForwardIterator>::with_zone& o, const std::list<T>& v)
{
	o.type = type::ARRAY;
	if(v.empty()) {
