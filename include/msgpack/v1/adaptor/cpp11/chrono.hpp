...
struct as<std::chrono::system_clock::time_point> {
    typename std::chrono::system_clock::time_point operator()(msgpack::object const& o) const {
        ...
        case 12: {
            ...
            uint64_t sec;
            _msgpack_load64(uint64_t, o.via.ext.data(), &sec);
            tp += std::chrono::nanoseconds(nanosec);
            tp += std::chrono::seconds(sec);
        } break;
        default:
...
struct pack<std::chrono::system_clock::time_point> {
    ...
    msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const std::chrono::system_clock::time_point& v) const {
        uint64_t nanosec = std::chrono::duration_cast<std::chrono::nanoseconds>(v.time_since_epoch()).count()
            % 1000000000;
        uint64_t sec = std::chrono::duration_cast<std::chrono::seconds>(v.time_since_epoch()).count();
